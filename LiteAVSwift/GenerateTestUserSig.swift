 ...
 */
let TX_BIZID:Int32 = 0

let TX_APPID:Int32 = 0
 ...
 * 它是腾讯云用于区分客户的唯一标识。
 */
let _SDKAppID:Int32 = <#T##sdkAppid: 14xxxxxxxx###>
...
*/
let _SECRETKEY:String = <#T##SECRETKEY: String###>

class GenerateTestUserSig: NSObject {
    /**
    * 计算 UserSig 签名
    *
    ...
    */
    ...
    class func genTestUserSig(_ identifier:String)->String{
        return genTestUserSig(identifier, sdkAppId: Int(_SDKAppID), secretKey: _SECRETKEY)
    }
    
    class func genTestUserSig(_ identifier:String,sdkAppId:Int,secretKey:String)->String{
        
       let current:CFTimeInterval = CFAbsoluteTimeGetCurrent() + kCFAbsoluteTimeIntervalSince1970
        let  TLSTime:CLong = CLong(floor(current))
       var obj:Dictionary<String,Any> = ["TLS.ver": "2.0",
        ...
        for key in keyOrder {
            stringToSign = stringToSign.appending("\(key):\(obj[key]!)\n")
        }
        debugPrint(stringToSign)
        
        let sig = hmac(plainText: stringToSign, key: secretKey)
        obj["TLS.sig"] = sig
        debugPrint("sig: \(sig)")
        let jsonToZipData = obj.toData(dictionary: obj)
       
        let inputData = jsonToZipData
        var outputData = Data(count: Int(compressBound(uLong(inputData.count))))
        ...
        try? outputData.withUnsafeMutableBytes{ (outputBytes: UnsafeMutableRawBufferPointer) in
            ...
            try inputData.withUnsafeBytes{ (inputBytes: UnsafeRawBufferPointer) in
                ...
                guard compress2(outputBytes, &destLen, inputBytes, uLong(inputData.count), Z_BEST_SPEED) == Z_OK else {
        ...
        let result = base64URL(data: outputData)
        
        return result
    }
    
    class func hmac(plainText:String,key:String)->String{
        ...
        for oneChr in result{
            switch(oneChr){
                ...
                case "=":
                    ...
                    break;
                default:
                    finalStr = finalStr + String.init(oneChr)
                    break;
            }
        }
        return finalStr
